...
package object ResumeParser {
  ...
  def strategy_factory(root_node: Node): (Node => Boolean) = {
    ...
    def strategy(node: Node): Boolean = {
      val text = extract_simple_text_from_node(node)
      val text_result = !text.contains("/")
      val length_result = text.lengthCompare(1) > 0
      ...
      length_result && font_size_result && text_result
    }
